...
class Player():
    ...
    def __init__(self, screen, settings, id, turn=False):
        ...
        if id == 1:
            self.static_list = list(map(
                lambda x: [-x[0], 0],
                self.static_list)
            )
            self.moving_list = list(map(
                lambda x: [-x[0], 0],
                self.moving_list)
            )
    ...
    def update(self):
        ...
        for i in enumerate(self.static_list):
            if (self.static_list[i[0]][1] == 0 and
                    pos_y < self.static_list[i[0]][0]):
        ...
        for i in enumerate(self.moving_list):
            if (self.moving_list[i[0]][1] == 0 and
                    pos_y < self.moving_list[i[0]][0]):
    ...
    def reset(self):
        ...
        if self.alive:
            ...
            self.static_list = [[215, 0], [385, 0], [565, 0], [740, 0]]
            self.moving_list = [
                [125, 0], [300, 0], [475, 0], [650, 0], [830, 0]
            ]

            # Convert to negative so as to make checking easy for player 2
            if self.id == 1:
                self.static_list = list(map(
                    lambda x: [-x[0], 0], self.static_list)
                )
                self.moving_list = list(map(
                    lambda x: [-x[0], 0], self.moving_list)
                )
